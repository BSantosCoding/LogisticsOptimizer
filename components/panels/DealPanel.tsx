...
import { Plus, Save, Pencil, Trash2, X, Scale, Box, DollarSign, Clock, MapPin, ShieldAlert, Search } from 'lucide-react';
...
const DealPanel: React.FC<DealPanelProps> = ({
  ...
  const filteredDeals = deals.filter(d =>
    d.carrierName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <>
      <div className={`p-4 border-b border-slate-700 z-10 ${editingDealId ? 'bg-blue-900/10' : 'bg-slate-800'}`}>
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <input placeholder="Destination" value={newDeal.destination} onChange={e => setNewDeal({ ...newDeal, destination: e.target.value })} className="flex-1 bg-slate-900 border border-slate-600 rounded px-3 py-2 text-sm focus:border-blue-500 outline-none text-slate-200" />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        ...
        {filteredDeals.map(d => (
          <div key={d.id} className={`p-3 rounded border flex justify-between items-start group transition-colors ${editingDealId === d.id ? 'bg-blue-900/20 border-blue-500' : 'bg-slate-900/50 border-slate-700 hover:border-blue-500/30'}`}>
